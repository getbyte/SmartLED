...
typedef struct 
{
    ...
    inline const char* text()
    {
        ...
        sprintf(serialBuf, "\nCycle\nnextChange: %u\tperiod: %u\tcurrent: %u\tfading: %u\tisRandom: %d\tneedToFade: %d\n", nextChange, period, current, fading, isRandom, needToFade);
        return (const char*)serialBuf;
    }
} StripCycle;
...
typedef struct 
{
    ...
    inline const char* text()
    {
        char serialBuf[1024];
        memset(serialBuf, 0, 1024);
        sprintf(serialBuf, "\nWaves\ncolorMin: (%u:%u:%u)\tcolorMax: (%u:%u:%u)\tcount: (%u:%u:%u)\tspeed: (%d:%d:%d)\n", colorMin.r, colorMin.g, colorMin.b, colorMax.r, colorMax.g, colorMax.b, count.r, count.g, count.b, speed.r, speed.g, speed.b);
        return (const char*)serialBuf;
    }
} StripWaves;
...
typedef struct 
{
    ...
    inline const char* text()
    {
        ...
        {
            memset(tmpbuf, 0, 128);
            sprintf(tmpbuf, ((i > 0) ? "\t(%u:%u:%u)" : "(%u:%u:%u)"), color[i].r, color[i].g, color[i].b);
            strcat(serialBuf, tmpbuf);
        }
        memset(tmpbuf, 0, 128);
        sprintf(tmpbuf, "]\ncount: %u\tspeed: %d\treverse: %d\n", count, speed, reverse);
        strcat(serialBuf, tmpbuf);
        return (const char*)serialBuf;
    }
} StripRainbow;
...
typedef struct 
{
    ...
    inline const char* text()
    {
        ...
        memset(tmpbuf, 0, 128);
        sprintf(tmpbuf, "]\ncount: %d\tspeed: %u\treverse: %d\tmultiColor: %d\n", count, speed, reverse, multiColor);
        strcat(serialBuf, tmpbuf);
        return (const char*)serialBuf;
    }
} StripLines;
...
typedef struct 
{
    ...
    inline const char* text()
    {
        char serialBuf[1024];
        memset(serialBuf, 0, 1024);
        sprintf(serialBuf, "\nSnowflake\ncolor: (%u:%u:%u)\tmultiColor: %d\tflakeSize: %u\tcount: %u\tfading: %u\n", color.r, color.g, color.b, multiColor, flakeSize, count, fading);
        return (const char*)serialBuf;
    }
} StripSnowflake;
...
typedef struct 
{
    ...
    inline const char* text()
    {
        char serialBuf[1024];
        memset(serialBuf, 0, 1024);
        sprintf(serialBuf, "\nStroboscope\ncolor: (%u:%u:%u)\tmultiColor: %d\tcount: %u\n", color.r, color.g, color.b, multiColor, count);
        return (const char*)serialBuf;
    }
} StripStroboscope;
...
typedef struct 
{
    ...
    inline const char* text()
    {
        char serialBuf[1024];
        memset(serialBuf, 0, 1024);
        sprintf(serialBuf, "\nSnake\ncolor: (%u:%u:%u)\tcount: %u\tspeed: %d\tmultiColor: %d\treverse: %d\n", color.r, color.g, color.b, count, speed, multiColor, reverse);
        return (const char*)serialBuf;
    }
} StripSnake;
...
typedef struct 
{
    uint16_t pos;                         ///< текущая позиция точки
...
typedef struct 
{
    ...
    inline const char* text()
    {
        ...
        char tmpbuf[128];
        memset(serialBuf, 0, 1024);
        sprintf(serialBuf, "\nIntersects\ncount: %d\tpoint (pos, color, speed): [", count);
        for (int i = 0; i < 10; i++)
        {
            memset(tmpbuf, 0, 128);
            sprintf(tmpbuf, ((i > 0) ? "\t(%u\t%u:%u:%u\t%d)" : "(%u\t%u:%u:%u\t%d)"), point[i].pos, point[i].color.r, point[i].color.g, point[i].color.b, point[i].speed);
            strcat(serialBuf, tmpbuf);
        }
        strcat(serialBuf, "]\n");
        return (const char*)serialBuf;
...
typedef struct 
{
    ...
    inline const char* text()
    {
        char serialBuf[1024];
        memset(serialBuf, 0, 1024);
        sprintf(serialBuf, "\nPulse\ncolorMin: (%u:%u:%u)\tcolorMax: (%u:%u:%u)\tspeed: %d\n", colorMin.r, colorMin.g, colorMin.b, colorMax.r, colorMax.g, colorMax.b, speed);
        return (const char*)serialBuf;
    }
} StripPulse;
...
typedef struct
{
    uint16_t discretization;             ///< дискретизация (или количество шагов)
    uint16_t currentDiscret;             ///< текущий дискрет (или шаг)
    ...
    inline const char* text()
    {
        char serialBuf[1024];
        memset(serialBuf, 0, 1024);
        sprintf(serialBuf, "\nModifierParameters\ndiscretization: %u\tcurrentDiscret: %u\tmodifierSpeed: %d\tmodifierVisible: %d\teffectPaused: %d\n", discretization, currentDiscret, modifierSpeed, modifierVisible, effectPaused);
        return (const char*)serialBuf;
    }
} ModifierParameters;
...
typedef struct 
{
    ...
    uint16_t schedulerSize;              ///< размер области памяти с запланированными эффектами
    inline const char* text()
    {
        char serialBuf[1024];
        memset(serialBuf, 0, 1024);
        sprintf(serialBuf, "\nConfiguration\nmode: %u\tspecialMode: %u\teffectCreating: %u\tdirect: %d\tposition: %d\tstep: %d\tcurrentColor: (%u:%u:%u)\theaderSize: %u\tschedulerSize: %u\n", (uint8_t)mode, (uint8_t)specialMode, effectCreating, direct, position, step, currentColor.r, currentColor.g, currentColor.b, headerSize, schedulerSize);
        return (const char*)serialBuf;
    }
} Configuration;
...
public:
     ...
     */
    SmartLED(uint16_t pCount, uint8_t pPin, neoPixelType colorScheme = NEO_RGB, bool ue = true);
    /**
     * Деструктор класса
     */
    ~SmartLED();
     ...
     * @return текст со значениями ячек памяти
     */
    const char* memoryDump(uint8_t* begin, uint8_t length);
    /**
     * Сделать дамп памяти
     */
    void dump(uint8_t num);
