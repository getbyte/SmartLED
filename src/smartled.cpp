...
void webSocketEvent(uint16_t num, WStype_t type, uint8_t * buffer, size_t len)
{
    char incoming[len+2];
...
SmartLED::SmartLED(uint16_t pCount, uint8_t pPin, neoPixelType colorScheme, bool ue)
{
    defaultSpeed = 100;
    modifier = 0;
    zeroSpeed = 0;
...
void SmartLED::process()
{
    ...
    } else if ( nextStep < 1000 && currentMicros > 1000 ) 
    {
        microsOverflow = false;          
    }
...
void SmartLED::autosave()
{
    ...
    settings.schedulerSize = sizeof (sheduler);
    uint8_t eeprom_buffer[settings.headerSize + settings.schedulerSize];
    EEPROM.begin(sizeof (eeprom_buffer));

    memcpy(eeprom_buffer, &settings, settings.headerSize);
    memcpy(eeprom_buffer + settings.headerSize, &sheduler, settings.schedulerSize);
...
bool SmartLED::loadSettings()
{
    ...
    if ((settings.headerSize != sizeof (settings)) || (settings.schedulerSize != sizeof (sheduler)))
        return false;
...
const char* SmartLED::memoryDump(uint8_t* begin, uint8_t length)
{
    char tmpbuf[2048];
    char tmps[16];
    ...
    {
        sprintf(tmps, "%u\t", begin[i]);
        strcat(tmpbuf, tmps);
        if (i % 10 == 9) strcat(tmpbuf, "\n");
    }
    Serial.printf("%s", tmpbuf);
    return (const char*)tmpbuf;
}

void SmartLED::dump(uint8_t num)
{
    sendTXT(num, settings.waves.text());
    sendTXT(num, settings.rainbow.text());
    sendTXT(num, settings.lines.text());
    sendTXT(num, settings.snowflake.text());
    sendTXT(num, settings.stroboscope.text());
    sendTXT(num, settings.intersects.text());
    sendTXT(num, settings.snake.text());
    sendTXT(num, settings.pulse.text());
    sendTXT(num, settings.cycle.text());
    sendTXT(num, modSettings.text());
    sendTXT(num, settings.text());
    Serial.printf(settings.waves.text());
    Serial.printf(settings.rainbow.text());
    Serial.printf(settings.lines.text());
    Serial.printf(settings.snowflake.text());
    Serial.printf(settings.stroboscope.text());
    Serial.printf(settings.intersects.text());
    Serial.printf(settings.snake.text());
    Serial.printf(settings.pulse.text());
    Serial.printf(settings.cycle.text());
    Serial.printf(modSettings.text());
    Serial.printf(settings.text());
}
